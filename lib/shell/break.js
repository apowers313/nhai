...
function createBreakpoint(opts) {
    ...
    opts.all = true;
    let bp = new Breakpoint(opts, name);

    // console.log("Breakpoint added:", bp.toString());
}
...
function incrNumbers(names) {
    let newNames = names.map((name) => (typeof name === "number") ? ++name : name);
    names.splice(0, names.length, ... newNames);
}
