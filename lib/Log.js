...
let stdoutStream = bunyanDebugStream({
...
defaultLogger.trace("Logger starting:", Config.get("app-name"), Config.get("version"));
...
class Log {
    ...
    static setStdoutLevel(level) {
        if (typeof level === "string") level = Log.nameToLevel(level);

        if ((typeof level !== "number") ||
            (level > 60) ||
            (level < 10) ||
            ((level % 10) !== 0)) throw new TypeError("setStdoutLevel expected level to be a number and a multiple of 10 between 0 and 61");
