...
class Synchronize {
     ...
     */
    static init() {
        console.print("Synchronize: init");
        tickCount = 0;
     ...
     */
    static syncWatchdog() {
        console.print(`Synchronize: syncWatchdog had tick: ${hadTick} (${process.hrtime()})`);
        if (!hadTick) {
...
async function _nextTick() {
    console.print("Synchronize: _nextTick", process.hrtime());
    tickCount++;
